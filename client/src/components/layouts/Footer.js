      ...
      <ul className="ft-link">
        {footerLink &&
          footerLink.map((item, index) => {
            return (
              <li key={item.title} className="ft-link-item">
                <h6 className="ft-link-tt">{item.title}</h6>
                {item?.listLink &&
                  item?.listLink.map((childItem, childIndex) => {
                    return (
                      <small key={childItem} className="ft-link-it">
                        {childItem}
                      </small>
                    );
                  })}
      ...
      <ul className="ft-contact">
        ...
        {footerContact &&
          footerContact.map((item, index) => {
            return (
              <li key={item.title} className="ft-contact-mn">
